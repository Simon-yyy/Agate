...
- [pkg/guard/preflight.go](pkg/guard/preflight.go)：Phase 0 静态合规拦截引擎，包含 7 大检查卡口：
  ...
  6. vendor 依赖目录纯净度；
  7. 3-Hop 双地图完备性校验（MAP.md 与 contexts/context.md 成对存在）。
...
- [pkg/harness/scaffold.go](pkg/harness/scaffold.go)：初始化 `.ignore`、`MAP.md` 与 `contexts/context.md` 模板渲染与落盘。
...
1. **第 1 跳 (全局拓扑)**：阅读 [MAP.md](MAP.md)，定位功能模块归属与关键文件；
