...
- 输出架构拓扑报告，支持通过 --write 自动丰富 AGENTS.md。`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if flagWriteScan {
			return appendTopologyToAgents(topo)
		}

		fmt.Println("\n💡 提示：运行 `agate scan --write` 可将上述扫描结果自动合并回填至 AGENTS.md")
		return nil
	},
}
...
func appendTopologyToAgents(topo *ProjectTopology) error {
	...
	fmt.Println("\n\033[92m[+] 扫描结果已成功合并回填至 AGENTS.md\033[0m")
	return nil
}

func init() {
	scanCmd.Flags().BoolVarP(&flagWriteScan, "write", "w", false, "将扫描出的技术拓扑自动回填至 AGENTS.md")
